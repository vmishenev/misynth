...
#define REPORT(x) if (reporter && reporter->get_stream()) (*(reporter->get_stream())) << x
...
namespace Duality {
    ...
    public:
        ...
        virtual std::ostream *get_stream() {return 0;}
        virtual ~Reporter(){}
    };
    ...
    public:
        ...
        */
        ...
        public:
            ...
            virtual std::vector<RPFP::Transformer>& ProposeStrong(Node *node) = 0;
            virtual ~Proposer(){};
            bool chatty;
        };
        ...
        int unwinding_nodes_at_last_full_scan;
        ...
        void PreSolve(){
            ...
            unwinding_nodes_at_last_full_scan = -1;
        }
        ...
        virtual void Conjecture(const std::vector<expr> &conjs){
            proposers.push_back(make_conjecture_proposer(conjs,this));
        }
        ...
        */
        ...
        void GenCandidatesFromInductionFailure(bool full_scan = false){
            ...
            if(!full_scan && candidates.empty()){
                if (unwinding_nodes_at_last_full_scan == (int)unwinding->nodes.size())
                    reporter->Message("No nodes since last full scan. We are inductive.");
                else {
                    reporter->Message("No candidates from updates. Trying full scan.");
                    unwinding_nodes_at_last_full_scan = unwinding->nodes.size();
            ...
            for(unsigned i = 0; i < proposers.size(); i++){
                ...
                for(unsigned j = 0; j < conjectures.size(); j++){ 
                    ...
                    for(unsigned k = 0; k < conj_vec.size(); k++){
                        ...
                        if (proposers[i]->chatty)
                            REPORT("trying conjecture: " << bound.Formula  << "\n");
                ...
                const std::vector<RPFP::Transformer> strong_conjectures = proposers[i]->ProposeStrong(node);
                for(unsigned j = 0; j < strong_conjectures.size(); j++){ 
                    reporter->Message("Trying strong conjecture.");
                    if(ProveConjecture(node,strong_conjectures[j]))
                        reporter->Message("Conjecture holds.");
                    else
                        reporter->Message("Conjecture does not hold.");
                }
            }
        }
...
#define NO_CONJ_ON_SIMPLE_LOOPS
        ...
        */
        ...
        class HistoryProposer : public Proposer
        {
            Duality *old_solver;
            Duality *new_solver;
            hash_map<Node *, std::vector<RPFP::Transformer> > conjectures, strong_conjectures;

        public:
            /** Construct a history solver. */
            HistoryProposer(Duality *_old_solver, Duality *_new_solver, bool _chatty = false)
                ...
                for(unsigned i = 0; i < rpfp->nodes.size(); i++)
                    for(unsigned j = 0; j < wildcards.size(); j++){
                        Node *node = wildcards[j];
                        std::string key = GetKey(node);
                        MatchNodes(rpfp->nodes[i],node,bckg_map,key.c_str());
                    }
            }
            ...
            virtual std::vector<RPFP::Transformer> &ProposeStrong(Node *node){
                return strong_conjectures[node->map];
            }
        ...
        private:
            void MatchNodes(Node *new_node, Node *old_node, hash_map<func_decl,func_decl> &bckg_map, const char *base = 0){
                Reporter *reporter = new_solver->reporter;
                ...
                if(chatty)
                    REPORT("conjecture match: " << fmla << "\n"); 
                std::string old_name = old_node->Name.name().str();
                // if conjecture name ends in asterisk, really check it
                if (old_name[old_name.size()-1] == '*')
                    strong_conjectures[new_node].push_back(new_annot);
                else
            ...
            std::string SubsBase(const std::string &key, const std::string &base, Node *node){
                std::string foo = key.substr(0,base.size());
                if (foo == base)
                    return GetKey(node) + key.substr(base.size());
                return key;
            }
        };

    /** This creates a HistoryProposer that gets its conjectures
        from a file rather than an old solver. The same name
        matching scheme is used (in fact it is implemented by constructing
        a fake solver with unwinding nodes from the file and using
        this to initialize HistoryProposer). The format of the file
        is a sequence of declarations of global constants, followed by
        predicate definitions using "define-fun". For example, the 
        conjecture that P(x) implies x > 0 is expressed as:

        (define-fun P ((x Int)) Bool (> x 0))
        
    */
    
        bool is_head_predicate(const expr &t){
            if(t.is_app() && t.decl().get_decl_kind() == Uninterpreted){
                int nargs = t.num_args();
                hash_set<ast> args_seen;
                for(int i = 0; i < nargs; i++){
                    expr arg = t.arg(i);
                    if(!arg.is_app() || !arg.num_args() == 0)
                        return false;
                    if(args_seen.find(arg) != args_seen.end())
                        return false;
                    args_seen.insert(arg);
                }
            }
            return true;
        }

        HistoryProposer *make_conjecture_proposer(const std::vector<expr> &conjs, Duality *new_solver){

            // This works by creating a fake "old" solver, in which
            // the conjectures are supposedly proved, then using HistoryProposer
            // to inherit them in the current solver.

            RPFP::LogicSolver *old_ls = RPFP::CreateLogicSolver(new_solver->ctx);
            RPFP *old_rpfp = new RPFP(old_ls);

                    
            Duality *old_solver = new Duality(old_rpfp);
            old_solver->unwinding = new RPFP(old_ls);
            ...
            for(unsigned i = 0; i < conjs.size(); i++){
                ...
                t = old_rpfp->SubstBound(subst,t);
                if(t.decl().get_decl_kind() == Implies){
                    expr head = t.arg(0);
                    expr body = t.arg(1);
                    if(is_head_predicate(head)) {
                        Node *node = old_solver->unwinding->CreateNode(head);
                        node->Annotation.Formula = body;
                    }
                }
            }
            HistoryProposer *res = new HistoryProposer(old_solver,new_solver, /*chatty = */ true);
            delete old_solver;
            delete old_rpfp;
            delete old_ls;
            return res;
    ...
    public:
        ...
        virtual std::ostream *get_stream() {return &s;}
